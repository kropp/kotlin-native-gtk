...
inline fun Container.infoBar(init: InfoBar.() -> Unit = {}): InfoBar = InfoBar().apply { init(); this@infoBar.add(this) }

private fun InfoBar_onClose_Handler(sender: CPointer<*>?, data: COpaquePointer?) {
    data?.asStableRef<InfoBar>()?.get()?.onClose?.invoke()
}
...
private fun InfoBar_onResponse_Handler(
    ...
    data?.asStableRef<InfoBar>()?.get()?.onResponse?.invoke(response_id)
}
...
open class InfoBar internal constructor(override val widgetPtr: CPointer<GtkWidget>? = null) : Box() {
    ...
    val onClose: Signal<InfoBar, CFunction<(CPointer<*>?, COpaquePointer?) -> Unit>> by lazy { Signal<InfoBar, CFunction<(CPointer<*>?, COpaquePointer?) -> Unit>>(widgetPtr!!, this, "close", staticCFunction(::InfoBar_onClose_Handler)) }
    ...
    val onResponse: Signal1<InfoBar, Int, CFunction<(
            ...
            COpaquePointer?
                    ...
                    Int,
                    COpaquePointer?
            ) -> Unit>>(widgetPtr!!, this, "response", staticCFunction(::InfoBar_onResponse_Handler)) }
    ...
    fun addActionWidget(child: CPointer<GtkWidget>?, response_id: Int): Unit = gtk_info_bar_add_action_widget(self, child, response_id)
     ...
     * signal on the message area when the widget is activated. The widget
     * is appended to the end of the message areas action area. */
    fun addActionWidget(child: Widget, response_id: Int): Unit = gtk_info_bar_add_action_widget(self, child.widgetPtr?.reinterpret(), response_id)
     ...
     * need it. */
    fun addButton(button_text: String, response_id: Int): CPointer<GtkButton>? = gtk_info_bar_add_button(self, button_text, response_id)!!.reinterpret()
    ...
    fun addButtons(first_button_text: String): Unit = gtk_info_bar_add_buttons(self, first_button_text)
    ...
    fun setResponseSensitive(response_id: Int, setting: Boolean): Unit = gtk_info_bar_set_response_sensitive(self, response_id, if (setting) gtk_true() else gtk_false())
    ...
    fun setDefaultResponse(response_id: Int): Unit = gtk_info_bar_set_default_response(self, response_id)
}
