...
private fun TreeViewColumn_onClicked_Handler(sender: CPointer<*>?, data: COpaquePointer?) {
    data?.asStableRef<TreeViewColumn>()?.get()?.onClicked?.invoke()
}
...
open class TreeViewColumn() {
    ...
    val onClicked: Signal<TreeViewColumn, CFunction<(CPointer<*>?, COpaquePointer?) -> Unit>> by lazy { Signal<TreeViewColumn, CFunction<(CPointer<*>?, COpaquePointer?) -> Unit>>(widgetPtr!!, this, "clicked", staticCFunction(::TreeViewColumn_onClicked_Handler)) }
    ...
    fun cellSetCellData(
            ...
            is_expander: Boolean,
            is_expanded: Boolean
    ): Unit = gtk_tree_view_column_cell_set_cell_data(self, tree_model, iter, if (is_expander) gtk_true() else gtk_false(), if (is_expanded) gtk_true() else gtk_false())
    ...
    fun clearAttributes(cell_renderer: CPointer<GtkCellRenderer>?): Unit = gtk_tree_view_column_clear_attributes(self, cell_renderer)
    ...
    fun focusCell(cell: CPointer<GtkCellRenderer>?): Unit = gtk_tree_view_column_focus_cell(self, cell)

    /**
     * Adds the @cell to end of the column. If @expand is %FALSE, then the @cell
     * is allocated no more space than it needs. Any unused space is divided
     * evenly between cells for which @expand is %TRUE. */
    fun packEnd(cell: CPointer<GtkCellRenderer>?, expand: Boolean): Unit = gtk_tree_view_column_pack_end(self, cell, if (expand) gtk_true() else gtk_false())

    /**
     * Packs the @cell into the beginning of the column. If @expand is %FALSE, then
     * the @cell is allocated no more space than it needs. Any unused space is divided
     * evenly between cells for which @expand is %TRUE. */
    fun packStart(cell: CPointer<GtkCellRenderer>?, expand: Boolean): Unit = gtk_tree_view_column_pack_start(self, cell, if (expand) gtk_true() else gtk_false())
    ...
    fun setAttributes(cell_renderer: CPointer<GtkCellRenderer>?): Unit = gtk_tree_view_column_set_attributes(self, cell_renderer)
}
