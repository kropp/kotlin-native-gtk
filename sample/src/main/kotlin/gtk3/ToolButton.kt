...
private fun ToolButton_onClicked_Handler(sender: CPointer<*>?, data: COpaquePointer?) {
    data?.asStableRef<ToolButton>()?.get()?.onClicked?.invoke()
}
...
@GtkDsl
open class ToolButton internal constructor(override val widgetPtr: CPointer<GtkWidget>? = null) : ToolItem() {
    ...
    val onClicked: Signal<ToolButton, CFunction<(CPointer<*>?, COpaquePointer?) -> Unit>> by lazy { Signal<ToolButton, CFunction<(CPointer<*>?, COpaquePointer?) -> Unit>>(widgetPtr!!, this, "clicked", staticCFunction(::ToolButton_onClicked_Handler)) }
     ...
     * label. */
    constructor(icon_widget: CPointer<GtkWidget>?, label: String) : this(gtk_tool_button_new(icon_widget?.reinterpret(), label)?.reinterpret())
}
