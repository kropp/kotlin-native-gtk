...
inline fun Container.colorButton(init: ColorButton.() -> Unit = {}): ColorButton = ColorButton().apply { init(); this@colorButton.add(this) }

private fun ColorButton_onColorSet_Handler(sender: CPointer<*>?, data: COpaquePointer?) {
    data?.asStableRef<ColorButton>()?.get()?.onColorSet?.invoke()
}
...
@GtkDsl
open class ColorButton internal constructor(override val widgetPtr: CPointer<GtkWidget>? = null) : Button() {
    ...
    val onColorSet: Signal<ColorButton, CFunction<(CPointer<*>?, COpaquePointer?) -> Unit>> by lazy { Signal<ColorButton, CFunction<(CPointer<*>?, COpaquePointer?) -> Unit>>(widgetPtr!!, this, "color-set", staticCFunction(::ColorButton_onColorSet_Handler)) }
