...
inline fun Container.paned(orientation: GtkOrientation, init: Paned.() -> Unit = {}): Paned = Paned(orientation).apply { init(); this@paned.add(this) }

private fun Paned_onAcceptPosition_Handler(sender: CPointer<*>?, data: COpaquePointer?) {
    data?.asStableRef<Paned>()?.get()?.onAcceptPosition?.invoke()
}

private fun Paned_onCancelPosition_Handler(sender: CPointer<*>?, data: COpaquePointer?) {
    data?.asStableRef<Paned>()?.get()?.onCancelPosition?.invoke()
}

private fun Paned_onToggleHandleFocus_Handler(sender: CPointer<*>?, data: COpaquePointer?) {
    data?.asStableRef<Paned>()?.get()?.onToggleHandleFocus?.invoke()
}
 ...
 * ]| */
@GtkDsl
open class Paned internal constructor(override val widgetPtr: CPointer<GtkWidget>? = null) : Container() {
    ...
    val onAcceptPosition: Signal<Paned, CFunction<(CPointer<*>?, COpaquePointer?) -> Unit>> by lazy { Signal<Paned, CFunction<(CPointer<*>?, COpaquePointer?) -> Unit>>(widgetPtr!!, this, "accept-position", staticCFunction(::Paned_onAcceptPosition_Handler)) }

    val onCancelPosition: Signal<Paned, CFunction<(CPointer<*>?, COpaquePointer?) -> Unit>> by lazy { Signal<Paned, CFunction<(CPointer<*>?, COpaquePointer?) -> Unit>>(widgetPtr!!, this, "cancel-position", staticCFunction(::Paned_onCancelPosition_Handler)) }

    val onToggleHandleFocus: Signal<Paned, CFunction<(CPointer<*>?, COpaquePointer?) -> Unit>> by lazy { Signal<Paned, CFunction<(CPointer<*>?, COpaquePointer?) -> Unit>>(widgetPtr!!, this, "toggle-handle-focus", staticCFunction(::Paned_onToggleHandleFocus_Handler)) }
    ...
    fun pack1(
            ...
            resize: Boolean,
            shrink: Boolean
    ): Unit = gtk_paned_pack1(self, child, if (resize) gtk_true() else gtk_false(), if (shrink) gtk_true() else gtk_false())
    ...
    fun pack1(
            ...
            resize: Boolean,
            shrink: Boolean
    ): Unit = gtk_paned_pack1(self, child.widgetPtr?.reinterpret(), if (resize) gtk_true() else gtk_false(), if (shrink) gtk_true() else gtk_false())
    ...
    fun pack2(
            ...
            resize: Boolean,
            shrink: Boolean
    ): Unit = gtk_paned_pack2(self, child, if (resize) gtk_true() else gtk_false(), if (shrink) gtk_true() else gtk_false())
    ...
    fun pack2(
            ...
            resize: Boolean,
            shrink: Boolean
    ): Unit = gtk_paned_pack2(self, child.widgetPtr?.reinterpret(), if (resize) gtk_true() else gtk_false(), if (shrink) gtk_true() else gtk_false())
}
