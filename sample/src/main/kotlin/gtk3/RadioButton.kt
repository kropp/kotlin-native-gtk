...
inline fun Container.radioButton(group: CPointer<GSList>?, init: RadioButton.() -> Unit = {}): RadioButton = RadioButton(group).apply { init(); this@radioButton.add(this) }

private fun RadioButton_onGroupChanged_Handler(sender: CPointer<*>?, data: COpaquePointer?) {
    data?.asStableRef<RadioButton>()?.get()?.onGroupChanged?.invoke()
}
...
@GtkDsl
open class RadioButton internal constructor(override val widgetPtr: CPointer<GtkWidget>? = null) : CheckButton() {
    ...
    val onGroupChanged: Signal<RadioButton, CFunction<(CPointer<*>?, COpaquePointer?) -> Unit>> by lazy { Signal<RadioButton, CFunction<(CPointer<*>?, COpaquePointer?) -> Unit>>(widgetPtr!!, this, "group-changed", staticCFunction(::RadioButton_onGroupChanged_Handler)) }
    ...
    constructor(group: CPointer<GSList>?) : this(gtk_radio_button_new(group?.reinterpret()))
     ...
     *     }
     * ]| */
    fun joinGroup(group_source: CPointer<GtkRadioButton>?): Unit = gtk_radio_button_join_group(self, group_source)
     ...
     *     }
     * ]| */
    fun joinGroup(group_source: RadioButton): Unit = gtk_radio_button_join_group(self, group_source.widgetPtr?.reinterpret())
}
