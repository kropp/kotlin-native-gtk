...
inline fun Container.label(str: String, init: Label.() -> Unit = {}): Label = Label(str).apply { init(); this@label.add(this) }

private fun Label_onActivateCurrentLink_Handler(sender: CPointer<*>?, data: COpaquePointer?) {
    data?.asStableRef<Label>()?.get()?.onActivateCurrentLink?.invoke()
}

private fun Label_onCopyClipboard_Handler(sender: CPointer<*>?, data: COpaquePointer?) {
    data?.asStableRef<Label>()?.get()?.onCopyClipboard?.invoke()
}
...
private fun Label_onPopulatePopup_Handler(
    ...
    data?.asStableRef<Label>()?.get()?.onPopulatePopup?.invoke(menu)
}
...
open class Label internal constructor(override val widgetPtr: CPointer<GtkWidget>? = null) : Misc() {
    ...
    val onActivateCurrentLink: Signal<Label, CFunction<(CPointer<*>?, COpaquePointer?) -> Unit>> by lazy { Signal<Label, CFunction<(CPointer<*>?, COpaquePointer?) -> Unit>>(widgetPtr!!, this, "activate-current-link", staticCFunction(::Label_onActivateCurrentLink_Handler)) }

    val onCopyClipboard: Signal<Label, CFunction<(CPointer<*>?, COpaquePointer?) -> Unit>> by lazy { Signal<Label, CFunction<(CPointer<*>?, COpaquePointer?) -> Unit>>(widgetPtr!!, this, "copy-clipboard", staticCFunction(::Label_onCopyClipboard_Handler)) }
    ...
    val onPopulatePopup: Signal1<Label, CPointer<GtkMenu>?, CFunction<(
            ...
            COpaquePointer?
                    ...
                    CPointer<GtkMenu>?,
                    COpaquePointer?
            ) -> Unit>>(widgetPtr!!, this, "populate-popup", staticCFunction(::Label_onPopulatePopup_Handler)) }
    ...
    fun selectRegion(start_offset: Int, end_offset: Int): Unit = gtk_label_select_region(self, start_offset, end_offset)
