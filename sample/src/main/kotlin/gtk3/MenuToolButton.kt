...
private fun MenuToolButton_onShowMenu_Handler(sender: CPointer<*>?, data: COpaquePointer?) {
    data?.asStableRef<MenuToolButton>()?.get()?.onShowMenu?.invoke()
}
 ...
 * ]| */
@GtkDsl
open class MenuToolButton internal constructor(override val widgetPtr: CPointer<GtkWidget>? = null) : ToolButton() {
    ...
    val onShowMenu: Signal<MenuToolButton, CFunction<(CPointer<*>?, COpaquePointer?) -> Unit>> by lazy { Signal<MenuToolButton, CFunction<(CPointer<*>?, COpaquePointer?) -> Unit>>(widgetPtr!!, this, "show-menu", staticCFunction(::MenuToolButton_onShowMenu_Handler)) }
     ...
     * @label as label. */
    constructor(icon_widget: CPointer<GtkWidget>?, label: String) : this(gtk_menu_tool_button_new(icon_widget?.reinterpret(), label)?.reinterpret())

    /**
     * Sets the tooltip markup text to be used as tooltip for the arrow button
     * which pops up the menu.  See gtk_tool_item_set_tooltip_text() for setting
     * a tooltip on the whole #GtkMenuToolButton. */
    fun setArrowTooltipMarkup(markup: String): Unit = gtk_menu_tool_button_set_arrow_tooltip_markup(self, markup)

    /**
     * Sets the tooltip text to be used as tooltip for the arrow button which
     * pops up the menu.  See gtk_tool_item_set_tooltip_text() for setting a tooltip
     * on the whole #GtkMenuToolButton. */
    fun setArrowTooltipText(text: String): Unit = gtk_menu_tool_button_set_arrow_tooltip_text(self, text)
}
