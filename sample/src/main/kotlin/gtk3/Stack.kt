...
inline fun Container.stack(init: Stack.() -> Unit = {}): Stack = Stack().apply { init(); this@stack.add(this) }
...
@GtkDsl
open class Stack internal constructor(override val widgetPtr: CPointer<GtkWidget>? = null) : Container() {
    ...
    fun addNamed(child: CPointer<GtkWidget>?, name: String): Unit = gtk_stack_add_named(self, child, name)

    /**
     * Adds a child to @stack.
     * The child is identified by the @name. */
    fun addNamed(child: Widget, name: String): Unit = gtk_stack_add_named(self, child.widgetPtr?.reinterpret(), name)
     ...
     * name. */
    fun getChildByName(name: String): CPointer<GtkWidget>? = gtk_stack_get_child_by_name(self, name)!!.reinterpret()
     ...
     * (see gtk_widget_show()) in order to become the visible
     * child of @stack. */
    fun setVisibleChildFull(name: String, transition: GtkStackTransitionType): Unit = gtk_stack_set_visible_child_full(self, name, transition)
}
