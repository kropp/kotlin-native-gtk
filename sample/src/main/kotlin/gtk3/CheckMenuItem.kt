...
inline fun Container.checkMenuItem(init: CheckMenuItem.() -> Unit = {}): CheckMenuItem = CheckMenuItem().apply { init(); this@checkMenuItem.add(this) }

private fun CheckMenuItem_onToggled_Handler(sender: CPointer<*>?, data: COpaquePointer?) {
    data?.asStableRef<CheckMenuItem>()?.get()?.onToggled?.invoke()
}
...
@GtkDsl
open class CheckMenuItem internal constructor(override val widgetPtr: CPointer<GtkWidget>? = null) : MenuItem() {
    ...
    val onToggled: Signal<CheckMenuItem, CFunction<(CPointer<*>?, COpaquePointer?) -> Unit>> by lazy { Signal<CheckMenuItem, CFunction<(CPointer<*>?, COpaquePointer?) -> Unit>>(widgetPtr!!, this, "toggled", staticCFunction(::CheckMenuItem_onToggled_Handler)) }
