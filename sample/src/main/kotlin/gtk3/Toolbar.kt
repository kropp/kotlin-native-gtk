...
inline fun Container.toolbar(init: Toolbar.() -> Unit = {}): Toolbar = Toolbar().apply { init(); this@toolbar.add(this) }
...
private fun Toolbar_onPopupContextMenu_Handler(
    ...
    data?.asStableRef<Toolbar>()?.get()?.onPopupContextMenu?.invoke(x, y, button)
}
...
@GtkDsl
open class Toolbar internal constructor(override val widgetPtr: CPointer<GtkWidget>? = null) : Container() {
    ...
    val onPopupContextMenu: Signal3<Toolbar, Int, Int, Int, CFunction<(
            ...
            COpaquePointer?
                    ...
                    Int,
                    COpaquePointer?
            ) -> Unit>>(widgetPtr!!, this, "popup-context-menu", staticCFunction(::Toolbar_onPopupContextMenu_Handler)) }
     ...
     * Returns the position of @item on the toolbar, starting from 0.
     * It is an error if @item is not a child of the toolbar. */
    fun getItemIndex(item: ToolItem): Int = gtk_toolbar_get_item_index(self, item.widgetPtr?.reinterpret())

    /**
     * Returns the @n'th item on @toolbar, or %NULL if the
     * toolbar does not contain an @n'th item. */
    fun getNthItem(n: Int): CPointer<GtkToolItem>? = gtk_toolbar_get_nth_item(self, n)!!.reinterpret()
     ...
     * 0 the item is prepended to the start of the toolbar. If @pos is
     * negative, the item is appended to the end of the toolbar. */
    fun insert(item: ToolItem, pos: Int): Unit = gtk_toolbar_insert(self, item.widgetPtr?.reinterpret(), pos)
     ...
     * toolbar. */
    fun setDropHighlightItem(tool_item: CPointer<GtkToolItem>?, index_: Int): Unit = gtk_toolbar_set_drop_highlight_item(self, tool_item, index_)
     ...
     * added to any widget hierarchy or used as highlight item for another
     * toolbar. */
    fun setDropHighlightItem(tool_item: ToolItem, index_: Int): Unit = gtk_toolbar_set_drop_highlight_item(self, tool_item.widgetPtr?.reinterpret(), index_)
