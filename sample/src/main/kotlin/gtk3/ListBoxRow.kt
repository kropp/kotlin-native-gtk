...
inline fun Container.listBoxRow(init: ListBoxRow.() -> Unit = {}): ListBoxRow = ListBoxRow().apply { init(); this@listBoxRow.add(this) }

private fun ListBoxRow_onActivate_Handler(sender: CPointer<*>?, data: COpaquePointer?) {
    data?.asStableRef<ListBoxRow>()?.get()?.onActivate?.invoke()
}

@GtkDsl
open class ListBoxRow internal constructor(override val widgetPtr: CPointer<GtkWidget>? = null) : Bin() {
    ...
    val onActivate: Signal<ListBoxRow, CFunction<(CPointer<*>?, COpaquePointer?) -> Unit>> by lazy { Signal<ListBoxRow, CFunction<(CPointer<*>?, COpaquePointer?) -> Unit>>(widgetPtr!!, this, "activate", staticCFunction(::ListBoxRow_onActivate_Handler)) }
