...
inline fun Container.expander(label: String, init: Expander.() -> Unit = {}): Expander = Expander(label).apply { init(); this@expander.add(this) }

private fun Expander_onActivate_Handler(sender: CPointer<*>?, data: COpaquePointer?) {
    data?.asStableRef<Expander>()?.get()?.onActivate?.invoke()
}
...
@GtkDsl
open class Expander internal constructor(override val widgetPtr: CPointer<GtkWidget>? = null) : Bin() {
    ...
    val onActivate: Signal<Expander, CFunction<(CPointer<*>?, COpaquePointer?) -> Unit>> by lazy { Signal<Expander, CFunction<(CPointer<*>?, COpaquePointer?) -> Unit>>(widgetPtr!!, this, "activate", staticCFunction(::Expander_onActivate_Handler)) }
