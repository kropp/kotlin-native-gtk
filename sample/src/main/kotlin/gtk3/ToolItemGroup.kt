...
inline fun Container.toolItemGroup(label: String, init: ToolItemGroup.() -> Unit = {}): ToolItemGroup = ToolItemGroup(label).apply { init(); this@toolItemGroup.add(this) }
...
@GtkDsl
open class ToolItemGroup internal constructor(override val widgetPtr: CPointer<GtkWidget>? = null) : Container() {
    ...
    fun getDropItem(x: Int, y: Int): CPointer<GtkToolItem>? = gtk_tool_item_group_get_drop_item(self, x, y)!!.reinterpret()

    /**
     * Gets the position of @item in @group as index. */
    fun getItemPosition(item: CPointer<GtkToolItem>?): Int = gtk_tool_item_group_get_item_position(self, item)

    /**
     * Gets the position of @item in @group as index. */
    fun getItemPosition(item: ToolItem): Int = gtk_tool_item_group_get_item_position(self, item.widgetPtr?.reinterpret())

    /**
     * Gets the tool item at @index in group. */
    fun getNthItem(index: Int): CPointer<GtkToolItem>? = gtk_tool_item_group_get_nth_item(self, index)!!.reinterpret()

    /**
     * Inserts @item at @position in the list of children of @group. */
    fun insert(item: CPointer<GtkToolItem>?, position: Int): Unit = gtk_tool_item_group_insert(self, item, position)

    /**
     * Inserts @item at @position in the list of children of @group. */
    fun insert(item: ToolItem, position: Int): Unit = gtk_tool_item_group_insert(self, item.widgetPtr?.reinterpret(), position)

    /**
     * Sets the position of @item in the list of children of @group. */
    fun setItemPosition(item: CPointer<GtkToolItem>?, position: Int): Unit = gtk_tool_item_group_set_item_position(self, item, position)

    /**
     * Sets the position of @item in the list of children of @group. */
    fun setItemPosition(item: ToolItem, position: Int): Unit = gtk_tool_item_group_set_item_position(self, item.widgetPtr?.reinterpret(), position)
}
