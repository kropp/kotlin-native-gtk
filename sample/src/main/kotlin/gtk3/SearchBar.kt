...
inline fun Container.searchBar(init: SearchBar.() -> Unit = {}): SearchBar = SearchBar().apply { init(); this@searchBar.add(this) }
...
@GtkDsl
open class SearchBar internal constructor(override val widgetPtr: CPointer<GtkWidget>? = null) : Bin() {
     ...
     * This is only required if the entry isn’t the direct child of the
     * search bar (as in our main example). */
    fun connectEntry(entry: Entry): Unit = gtk_search_bar_connect_entry(self, entry.widgetPtr?.reinterpret())
     ...
     * }
     * ]| */
    fun handleEvent(event: CPointer<GdkEvent>?): Boolean = gtk_search_bar_handle_event(self, event) != 0
}
