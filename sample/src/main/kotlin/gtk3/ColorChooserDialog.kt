...
): ColorChooserDialog = ColorChooserDialog(title, parent).apply { init(); this@colorChooserDialog.add(this) }

/**
 * The #GtkColorChooserDialog widget is a dialog for choosing
 * a color. It implements the #GtkColorChooser interface. */
@GtkDsl
open class ColorChooserDialog internal constructor(override val widgetPtr: CPointer<GtkWidget>? = null) : Dialog() {
    ...
    constructor(title: String, parent: CPointer<GtkWindow>?) : this(gtk_color_chooser_dialog_new(title, parent?.reinterpret())?.reinterpret())
}
