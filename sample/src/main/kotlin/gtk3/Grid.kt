...
inline fun Container.grid(init: Grid.() -> Unit = {}): Grid = Grid().apply { init(); this@grid.add(this) }
...
@GtkDsl
open class Grid internal constructor(override val widgetPtr: CPointer<GtkWidget>? = null) : Container() {
    ...
    fun attachNextTo(
            ...
            width: Int,
            height: Int
    ): Unit = gtk_grid_attach_next_to(self, child.widgetPtr?.reinterpret(), sibling.widgetPtr?.reinterpret(), side, width, height)

    /**
     * Gets the child of @grid whose area covers the grid
     * cell whose upper left corner is at @left, @top. */
    fun getChildAt(left: Int, top: Int): CPointer<GtkWidget>? = gtk_grid_get_child_at(self, left, top)!!.reinterpret()

    /**
     * Returns the baseline position of @row as set
     * by gtk_grid_set_row_baseline_position() or the default value
     * %GTK_BASELINE_POSITION_CENTER. */
    fun getRowBaselinePosition(row: Int): GtkBaselinePosition = gtk_grid_get_row_baseline_position(self, row)
    ...
    fun insertNextTo(sibling: CPointer<GtkWidget>?, side: GtkPositionType): Unit = gtk_grid_insert_next_to(self, sibling, side)
     ...
     * a row is inserted. If @side is %GTK_POS_LEFT of %GTK_POS_RIGHT,
     * a column is inserted. */
    fun insertNextTo(sibling: Widget, side: GtkPositionType): Unit = gtk_grid_insert_next_to(self, sibling.widgetPtr?.reinterpret(), side)
    ...
    fun setRowBaselinePosition(row: Int, pos: GtkBaselinePosition): Unit = gtk_grid_set_row_baseline_position(self, row, pos)
}
