...
private fun SpinButton_onInput_Handler(
    ...
    data?.asStableRef<SpinButton>()?.get()?.onInput?.invoke(new_value)
}

private fun SpinButton_onOutput_Handler(sender: CPointer<*>?, data: COpaquePointer?) {
    data?.asStableRef<SpinButton>()?.get()?.onOutput?.invoke()
}

private fun SpinButton_onValueChanged_Handler(sender: CPointer<*>?, data: COpaquePointer?) {
    data?.asStableRef<SpinButton>()?.get()?.onValueChanged?.invoke()
}

private fun SpinButton_onWrapped_Handler(sender: CPointer<*>?, data: COpaquePointer?) {
    data?.asStableRef<SpinButton>()?.get()?.onWrapped?.invoke()
}
 ...
 * ]| */
@GtkDsl
open class SpinButton internal constructor(override val widgetPtr: CPointer<GtkWidget>? = null) : Entry() {
    ...
    val onOutput: Signal<SpinButton, CFunction<(CPointer<*>?, COpaquePointer?) -> Unit>> by lazy { Signal<SpinButton, CFunction<(CPointer<*>?, COpaquePointer?) -> Unit>>(widgetPtr!!, this, "output", staticCFunction(::SpinButton_onOutput_Handler)) }

    val onValueChanged: Signal<SpinButton, CFunction<(CPointer<*>?, COpaquePointer?) -> Unit>> by lazy { Signal<SpinButton, CFunction<(CPointer<*>?, COpaquePointer?) -> Unit>>(widgetPtr!!, this, "value-changed", staticCFunction(::SpinButton_onValueChanged_Handler)) }

    val onWrapped: Signal<SpinButton, CFunction<(CPointer<*>?, COpaquePointer?) -> Unit>> by lazy { Signal<SpinButton, CFunction<(CPointer<*>?, COpaquePointer?) -> Unit>>(widgetPtr!!, this, "wrapped", staticCFunction(::SpinButton_onWrapped_Handler)) }
    ...
    fun configure(
            ...
            climb_rate: Double,
            digits: Int
    ): Unit = gtk_spin_button_configure(self, adjustment.widgetPtr?.reinterpret(), climb_rate, digits)

    /**
     * Sets the step and page increments for spin_button.  This affects how
     * quickly the value changes when the spin button’s arrows are activated. */
    fun setIncrements(step: Double, page: Double): Unit = gtk_spin_button_set_increments(self, step, page)
    ...
    fun spin(direction: GtkSpinType, increment: Double): Unit = gtk_spin_button_spin(self, direction, increment)
