...
inline fun Container.toggleButton(init: ToggleButton.() -> Unit = {}): ToggleButton = ToggleButton().apply { init(); this@toggleButton.add(this) }

private fun ToggleButton_onToggled_Handler(sender: CPointer<*>?, data: COpaquePointer?) {
    data?.asStableRef<ToggleButton>()?.get()?.onToggled?.invoke()
}
 ...
 * ]| */
@GtkDsl
open class ToggleButton internal constructor(override val widgetPtr: CPointer<GtkWidget>? = null) : Button() {
    ...
    val onToggled: Signal<ToggleButton, CFunction<(CPointer<*>?, COpaquePointer?) -> Unit>> by lazy { Signal<ToggleButton, CFunction<(CPointer<*>?, COpaquePointer?) -> Unit>>(widgetPtr!!, this, "toggled", staticCFunction(::ToggleButton_onToggled_Handler)) }
