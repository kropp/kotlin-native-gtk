...
import libgtk3.GtkMenuDirectionType
...
private fun MenuShell_onCancel_Handler(sender: CPointer<*>?, data: COpaquePointer?) {
    data?.asStableRef<MenuShell>()?.get()?.onCancel?.invoke()
}

private fun MenuShell_onDeactivate_Handler(sender: CPointer<*>?, data: COpaquePointer?) {
    data?.asStableRef<MenuShell>()?.get()?.onDeactivate?.invoke()
}
...
private fun MenuShell_onInsert_Handler(
    ...
    data?.asStableRef<MenuShell>()?.get()?.onInsert?.invoke(child, position)
}
...
private fun MenuShell_onMoveSelected_Handler(
    ...
    data?.asStableRef<MenuShell>()?.get()?.onMoveSelected?.invoke(distance)
}

private fun MenuShell_onSelectionDone_Handler(sender: CPointer<*>?, data: COpaquePointer?) {
    data?.asStableRef<MenuShell>()?.get()?.onSelectionDone?.invoke()
}
...
abstract class MenuShell : Container() {
    ...
    val onCancel: Signal<MenuShell, CFunction<(CPointer<*>?, COpaquePointer?) -> Unit>> by lazy { Signal<MenuShell, CFunction<(CPointer<*>?, COpaquePointer?) -> Unit>>(widgetPtr!!, this, "cancel", staticCFunction(::MenuShell_onCancel_Handler)) }

    val onDeactivate: Signal<MenuShell, CFunction<(CPointer<*>?, COpaquePointer?) -> Unit>> by lazy { Signal<MenuShell, CFunction<(CPointer<*>?, COpaquePointer?) -> Unit>>(widgetPtr!!, this, "deactivate", staticCFunction(::MenuShell_onDeactivate_Handler)) }
    ...
    val onMoveSelected: Signal1<MenuShell, Int, CFunction<(
            ...
            COpaquePointer?
                    ...
                    Int,
                    COpaquePointer?
            ) -> Unit>>(widgetPtr!!, this, "move-selected", staticCFunction(::MenuShell_onMoveSelected_Handler)) }

    val onSelectionDone: Signal<MenuShell, CFunction<(CPointer<*>?, COpaquePointer?) -> Unit>> by lazy { Signal<MenuShell, CFunction<(CPointer<*>?, COpaquePointer?) -> Unit>>(widgetPtr!!, this, "selection-done", staticCFunction(::MenuShell_onSelectionDone_Handler)) }
    ...
    fun activateItem(menu_item: CPointer<GtkWidget>?, force_deactivate: Boolean): Unit = gtk_menu_shell_activate_item(self, menu_item, if (force_deactivate) gtk_true() else gtk_false())

    /**
     * Activates the menu item within the menu shell. */
    fun activateItem(menu_item: Widget, force_deactivate: Boolean): Unit = gtk_menu_shell_activate_item(self, menu_item.widgetPtr?.reinterpret(), if (force_deactivate) gtk_true() else gtk_false())
    ...
    fun bindModel(
            model: CPointer<libgtk3.GMenuModel>?,
            action_namespace: String,
            with_separators: Boolean
    ): Unit = gtk_menu_shell_bind_model(self, model, action_namespace, if (with_separators) gtk_true() else gtk_false())
    ...
    fun insert(child: CPointer<GtkWidget>?, position: Int): Unit = gtk_menu_shell_insert(self, child, position)

    /**
     * Adds a new #GtkMenuItem to the menu shell’s item list
     * at the position indicated by @position. */
    fun insert(child: Widget, position: Int): Unit = gtk_menu_shell_insert(self, child.widgetPtr?.reinterpret(), position)
     ...
     * item. */
    fun selectFirst(search_sensitive: Boolean): Unit = gtk_menu_shell_select_first(self, if (search_sensitive) gtk_true() else gtk_false())

    /**
     * Selects the menu item from the menu shell. */
    fun selectItem(menu_item: CPointer<GtkWidget>?): Unit = gtk_menu_shell_select_item(self, menu_item)

    /**
     * Selects the menu item from the menu shell. */
    fun selectItem(menu_item: Widget): Unit = gtk_menu_shell_select_item(self, menu_item.widgetPtr?.reinterpret())
}
