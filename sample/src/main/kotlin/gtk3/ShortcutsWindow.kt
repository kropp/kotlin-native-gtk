...
private fun ShortcutsWindow_onClose_Handler(sender: CPointer<*>?, data: COpaquePointer?) {
    data?.asStableRef<ShortcutsWindow>()?.get()?.onClose?.invoke()
}

private fun ShortcutsWindow_onSearch_Handler(sender: CPointer<*>?, data: COpaquePointer?) {
    data?.asStableRef<ShortcutsWindow>()?.get()?.onSearch?.invoke()
}
...
abstract class ShortcutsWindow : Window() {
    ...
    val onClose: Signal<ShortcutsWindow, CFunction<(CPointer<*>?, COpaquePointer?) -> Unit>> by lazy { Signal<ShortcutsWindow, CFunction<(CPointer<*>?, COpaquePointer?) -> Unit>>(widgetPtr!!, this, "close", staticCFunction(::ShortcutsWindow_onClose_Handler)) }

    val onSearch: Signal<ShortcutsWindow, CFunction<(CPointer<*>?, COpaquePointer?) -> Unit>> by lazy { Signal<ShortcutsWindow, CFunction<(CPointer<*>?, COpaquePointer?) -> Unit>>(widgetPtr!!, this, "search", staticCFunction(::ShortcutsWindow_onSearch_Handler)) }
}
