...
inline fun Container.flowBox(init: FlowBox.() -> Unit = {}): FlowBox = FlowBox().apply { init(); this@flowBox.add(this) }

private fun FlowBox_onActivateCursorChild_Handler(sender: CPointer<*>?, data: COpaquePointer?) {
    data?.asStableRef<FlowBox>()?.get()?.onActivateCursorChild?.invoke()
}
...
private fun FlowBox_onChildActivated_Handler(
    ...
    data?.asStableRef<FlowBox>()?.get()?.onChildActivated?.invoke(child)
}

private fun FlowBox_onSelectAll_Handler(sender: CPointer<*>?, data: COpaquePointer?) {
    data?.asStableRef<FlowBox>()?.get()?.onSelectAll?.invoke()
}

private fun FlowBox_onSelectedChildrenChanged_Handler(sender: CPointer<*>?, data: COpaquePointer?) {
    data?.asStableRef<FlowBox>()?.get()?.onSelectedChildrenChanged?.invoke()
}

private fun FlowBox_onToggleCursorChild_Handler(sender: CPointer<*>?, data: COpaquePointer?) {
    data?.asStableRef<FlowBox>()?.get()?.onToggleCursorChild?.invoke()
}

private fun FlowBox_onUnselectAll_Handler(sender: CPointer<*>?, data: COpaquePointer?) {
    data?.asStableRef<FlowBox>()?.get()?.onUnselectAll?.invoke()
}
...
@GtkDsl
open class FlowBox internal constructor(override val widgetPtr: CPointer<GtkWidget>? = null) : Container() {
    ...
    val onActivateCursorChild: Signal<FlowBox, CFunction<(CPointer<*>?, COpaquePointer?) -> Unit>> by lazy { Signal<FlowBox, CFunction<(CPointer<*>?, COpaquePointer?) -> Unit>>(widgetPtr!!, this, "activate-cursor-child", staticCFunction(::FlowBox_onActivateCursorChild_Handler)) }
    ...
    val onChildActivated: Signal1<FlowBox, CPointer<GtkFlowBoxChild>?, CFunction<(
            ...
            COpaquePointer?
                    ...
                    CPointer<GtkFlowBoxChild>?,
                    COpaquePointer?
            ) -> Unit>>(widgetPtr!!, this, "child-activated", staticCFunction(::FlowBox_onChildActivated_Handler)) }

    val onSelectAll: Signal<FlowBox, CFunction<(CPointer<*>?, COpaquePointer?) -> Unit>> by lazy { Signal<FlowBox, CFunction<(CPointer<*>?, COpaquePointer?) -> Unit>>(widgetPtr!!, this, "select-all", staticCFunction(::FlowBox_onSelectAll_Handler)) }

    val onSelectedChildrenChanged: Signal<FlowBox, CFunction<(CPointer<*>?, COpaquePointer?) -> Unit>> by lazy { Signal<FlowBox, CFunction<(CPointer<*>?, COpaquePointer?) -> Unit>>(widgetPtr!!, this, "selected-children-changed", staticCFunction(::FlowBox_onSelectedChildrenChanged_Handler)) }

    val onToggleCursorChild: Signal<FlowBox, CFunction<(CPointer<*>?, COpaquePointer?) -> Unit>> by lazy { Signal<FlowBox, CFunction<(CPointer<*>?, COpaquePointer?) -> Unit>>(widgetPtr!!, this, "toggle-cursor-child", staticCFunction(::FlowBox_onToggleCursorChild_Handler)) }

    val onUnselectAll: Signal<FlowBox, CFunction<(CPointer<*>?, COpaquePointer?) -> Unit>> by lazy { Signal<FlowBox, CFunction<(CPointer<*>?, COpaquePointer?) -> Unit>>(widgetPtr!!, this, "unselect-all", staticCFunction(::FlowBox_onUnselectAll_Handler)) }
    ...
    fun getChildAtIndex(idx: Int): CPointer<GtkFlowBoxChild>? = gtk_flow_box_get_child_at_index(self, idx)!!.reinterpret()

    /**
     * Gets the child in the (@x, @y) position. */
    fun getChildAtPos(x: Int, y: Int): CPointer<GtkFlowBoxChild>? = gtk_flow_box_get_child_at_pos(self, x, y)!!.reinterpret()
    ...
    fun insert(widget: CPointer<GtkWidget>?, position: Int): Unit = gtk_flow_box_insert(self, widget, position)
     ...
     * If @position is -1, or larger than the total number of children
     * in the @box, then the @widget will be appended to the end. */
    fun insert(widget: Widget, position: Int): Unit = gtk_flow_box_insert(self, widget.widgetPtr?.reinterpret(), position)
     ...
     * Selects a single child of @box, if the selection
     * mode allows it. */
    fun selectChild(child: FlowBoxChild): Unit = gtk_flow_box_select_child(self, child.widgetPtr?.reinterpret())
     ...
     * mode allows it. */
    fun unselectChild(child: CPointer<GtkFlowBoxChild>?): Unit = gtk_flow_box_unselect_child(self, child)

    /**
     * Unselects a single child of @box, if the selection
     * mode allows it. */
    fun unselectChild(child: FlowBoxChild): Unit = gtk_flow_box_unselect_child(self, child.widgetPtr?.reinterpret())
     ...
     * of the box. */
    fun setHadjustment(adjustment: CPointer<GtkAdjustment>?): Unit = gtk_flow_box_set_hadjustment(self, adjustment)
     ...
     * coordinate system as the allocation for immediate children
     * of the box. */
    fun setVadjustment(adjustment: CPointer<GtkAdjustment>?): Unit = gtk_flow_box_set_vadjustment(self, adjustment)
}
