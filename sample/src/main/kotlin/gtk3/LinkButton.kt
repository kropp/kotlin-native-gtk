...
inline fun Container.linkButton(uri: String, init: LinkButton.() -> Unit = {}): LinkButton = LinkButton(uri).apply { init(); this@linkButton.add(this) }

private fun LinkButton_onActivateLink_Handler(sender: CPointer<*>?, data: COpaquePointer?) {
    data?.asStableRef<LinkButton>()?.get()?.onActivateLink?.invoke()
}
...
@GtkDsl
open class LinkButton internal constructor(override val widgetPtr: CPointer<GtkWidget>? = null) : Button() {
    ...
    val onActivateLink: Signal<LinkButton, CFunction<(CPointer<*>?, COpaquePointer?) -> Unit>> by lazy { Signal<LinkButton, CFunction<(CPointer<*>?, COpaquePointer?) -> Unit>>(widgetPtr!!, this, "activate-link", staticCFunction(::LinkButton_onActivateLink_Handler)) }
