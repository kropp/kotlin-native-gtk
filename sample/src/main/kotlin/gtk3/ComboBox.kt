...
inline fun Container.comboBox(init: ComboBox.() -> Unit = {}): ComboBox = ComboBox().apply { init(); this@comboBox.add(this) }

private fun ComboBox_onChanged_Handler(sender: CPointer<*>?, data: COpaquePointer?) {
    data?.asStableRef<ComboBox>()?.get()?.onChanged?.invoke()
}

private fun ComboBox_onPopdown_Handler(sender: CPointer<*>?, data: COpaquePointer?) {
    data?.asStableRef<ComboBox>()?.get()?.onPopdown?.invoke()
}

private fun ComboBox_onPopup_Handler(sender: CPointer<*>?, data: COpaquePointer?) {
    data?.asStableRef<ComboBox>()?.get()?.onPopup?.invoke()
}
...
@GtkDsl
open class ComboBox internal constructor(override val widgetPtr: CPointer<GtkWidget>? = null) : Bin() {
    ...
    val onChanged: Signal<ComboBox, CFunction<(CPointer<*>?, COpaquePointer?) -> Unit>> by lazy { Signal<ComboBox, CFunction<(CPointer<*>?, COpaquePointer?) -> Unit>>(widgetPtr!!, this, "changed", staticCFunction(::ComboBox_onChanged_Handler)) }

    val onPopdown: Signal<ComboBox, CFunction<(CPointer<*>?, COpaquePointer?) -> Unit>> by lazy { Signal<ComboBox, CFunction<(CPointer<*>?, COpaquePointer?) -> Unit>>(widgetPtr!!, this, "popdown", staticCFunction(::ComboBox_onPopdown_Handler)) }

    val onPopup: Signal<ComboBox, CFunction<(CPointer<*>?, COpaquePointer?) -> Unit>> by lazy { Signal<ComboBox, CFunction<(CPointer<*>?, COpaquePointer?) -> Unit>>(widgetPtr!!, this, "popup", staticCFunction(::ComboBox_onPopup_Handler)) }
    ...
    fun popupForDevice(device: CPointer<GdkDevice>?): Unit = gtk_combo_box_popup_for_device(self, device)
